...
struct sensor_reg OV5642_QVGA[QVGAinit] =
{
   {0x00ff, 0x01},
   {0x3103 ,0x93},
   {0x3008 ,0x82},
...
struct sensor_reg OV5642_JPEG_TO_YUV[JPEGtoYUV] =
{
	{0x460c ,0x00},
	{0x460b ,0x37},
	{0x471c ,0xd0},
...
struct sensor_reg OV5642_JPEG_Capture_QSXGA[QSXGA] =
{
...
struct sensor_reg OV5642_QSXGA_TO_QVGA[QVGA] =
{
	{0x3800 ,0x1 },
	{0x3801 ,0xa8},
	{0x3802 ,0x0 },
...
void MyCam_Trigger(uint16_t rxBuffer, uint16_t txBuffer) {
	...
	for (k = 0; k < QSXGA; k++) {
			buffer[0] = OV5642_JPEG_Capture_QSXGA[k].reg >> 8;
			buffer[1] = OV5642_JPEG_Capture_QSXGA[k].reg;
			buffer[2] = OV5642_JPEG_Capture_QSXGA[k].val;
			Write_I2C(buffer, NUMBER_OF_I2C_ELEMENTS);
		}
	Cy_SysLib_Delay(1000);

	for (k = 0; k < QVGA; k++) {
		buffer[0] = OV5642_QSXGA_TO_QVGA[k].reg >> 8;
		buffer[1] = OV5642_QSXGA_TO_QVGA[k].reg;
		buffer[2] = OV5642_QSXGA_TO_QVGA[k].val;
		Write_I2C(buffer, NUMBER_OF_I2C_ELEMENTS);
	}
	Cy_SysLib_Delay(1000);
...
*******************************************************************************
*
* Summary: 		Sends trigger signal to have camera capture image
*
* Parameters: 	uint16_t *rxBuffer,
* 				uint16_t *txBuffer
*
...
void MyCam_Single_FIFO_Read(uint16_t txBuffer, uint16_t *ptr_PD) {
		...
		txBuffer = (((uint16_t)READ + (uint16_t)FIFO_READ_BURST) << 8) + (uint16_t)DUMMY;


		High_byte = Write_SPI(txBuffer) << 8;
		txBuffer = (((uint16_t)READ + (uint16_t)FIFO_READ_BURST) << 8) + (uint16_t)DUMMY;
		Low_byte = Write_SPI(txBuffer);

		*ptr_PD = High_byte + Low_byte;
}
...
void MyCam_Init(void) {
	...
	for (k = 0; k < QVGAinit; k++) {
		buffer[0] =	OV5642_QVGA[k].reg >> 8;
		buffer[1] = OV5642_QVGA[k].reg;
		buffer[2] = OV5642_QVGA[k].val;
		Write_I2C(buffer, NUMBER_OF_I2C_ELEMENTS);
	}
	Cy_SysLib_Delay(250);
};
